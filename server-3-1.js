...
// app.use(verifyCsrf);   // CSRF devre dışı
...
function generateTokens(user) {
    ...
    const accessToken = jwt.sign(
        ...
        JWT_SECRET,
        { expiresIn: '7d', algorithm: 'HS256' } // 7 gün: refresh başarısız olduğunda bile uzun süre geçerli
    );
...
function setAuthCookies(res, req, tokens) {
    ...
    res.cookie('access_token', tokens.accessToken, {
        ...
        sameSite : 'strict',
        path     : '/',
        maxAge   : 7 * 24 * 60 * 60 * 1000, // 7 gün — JWT expiry ile eşleştirildi
    });
...
function setCsrfCookie(res, req, token) {
    ...
    res.cookie('csrf_token', token, {
        ...
        sameSite : 'strict',
        path     : '/',
        maxAge   : 7 * 24 * 60 * 60 * 1000, // 7 gün (access token ile eşleşti)
    });
}
...
<p>Bu gönderi silinmiş veya mevcut değil.</p><a href="${DOMAIN}">Ana Sayfaya Dön</a></div></body></html>`);
        ...
        if (mediaItems.length > 1) {
            const slides = mediaItems.map((item, idx) =>
                item.type === 'video'
                ? `<div class="slide" id="slide-${idx}" style="display:${idx===0?'block':'none'}"><video controls playsinline muted autoplay loop style="width:100%;max-height:520px;background:#000;display:block;object-fit:contain" src="${item.url}" onerror="this.parentElement.style.display='none'"></video></div>`
